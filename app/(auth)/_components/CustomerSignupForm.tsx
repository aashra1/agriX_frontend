...
export const CustomerRegisterForm = ({ setRole }: { setRole: (role: 'initial') => void }) => {
...
const onSubmit = async (data: RegisterFields) => {
  ...
  try {
    const payload = { 
      ...
      confirmPassword: data.confirmPassword.trim(),  // Add trimmed confirmPassword to the payload
    };
  ...
  const InputField = ({ name, label, type = "text", iconPath, error, isPassword, show, toggleShow }: 
    { name: keyof RegisterFields; label: string; type?: string; iconPath: string; error: string | undefined; isPassword?: boolean; show?: boolean; toggleShow?: () => void; }) => (
